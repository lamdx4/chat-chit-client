...
export function StoryArchive({
  ...
  const nextStory = useCallback(async () => {
    if (
      ...
      const story = localStories[newIndex];
      if (story && onStoryClick) {
        await onStoryClick(story.storyId);
      }
    }
  }, [selectedStoryIndex, localStories, onStoryClick]);
  ...
  const currentStory =
    selectedStoryIndex !== null ? localStories[selectedStoryIndex] : null;

  const handleHeartClick = useCallback(
    async (e: React.MouseEvent, storyId: number) => {
      e.stopPropagation();

      const currentStory = localStories.find(
    ...
    [localStories, onHeartClick]
  );

  const handleRemoveFromArchive = useCallback(async () => {
    const currentStory = localStories[selectedStoryIndex];
    if (!currentStory || !onRemoveFromArchive) return;

    setIsMenuOpen(false);
    await onRemoveFromArchive(currentStory.storyId);
    closeModal();
  }, [localStories, selectedStoryIndex, onRemoveFromArchive]);
                  ...
                  <div className="text-white">
                    ...
                    <div className="flex justify-start">
                      <button
                        ...
                        onClick={(e) =>
                          handleHeartClick(e, currentStory.storyId)
                        }
                      >
                        <svg
                        ...
                        <span className="text-sm font-medium">
                          {Number(currentStory.reactCount)}
                        </span>
                      </button>
                    </div>
                  </div>
